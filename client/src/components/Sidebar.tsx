...
const Sidebar = ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (val: boolean) => void }) => {
  ...
  const navItems = [
    ...
    { name: "AIComposer", icon: Wand2Icon, path: "/ai-composer" }
  ];
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink key={item.name}
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm transition-all duration-150 border ${isActive ? "bg-red-50 border-red-100 text-red-600" : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"}`}>

              <item.icon className={`size-4.5 shrink-0 ${isActive ? "text-red-500" : "text-slate-500"}`} />
              {item.name}
              {isActive && <span className="ml-auto w-1.25px h-5 rounded-full bg-red-500" />}
            </NavLink>
          )
        })}
      </nav>
      ...
      <div className="p-4 border-t border-slate-100">
        ...
        <button onClick={logout} className="mt-1 flex items-center gap-2 px-3 py-2 rounded text-sm text-slate-500 hover:bg-red-50 hover:text-red-500 transition-all duration-150 w-full">
