  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        <h1 className="text-3xl font-bold tracking-tight">What should we create today?</h1>
        <div className="relative group mt-12">
          ...
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            <button onClick={() => setGenerateImage(!generateImage)} className="flex
              items-center gap-2 bg-red-50 py-2 px-3 rounded-lg">
              <span>AI Image</span>
...
rounded-full bg-white shadow transition ${generateImage ? "translate-x-4.5" :
                    "translate-x-0.5"}`} />
              </div>
            </button>
            <button onClick={handleGenerate} disabled={loading} className="bg-slate-900
hover:bg-slate-800 text-white flex items-center gap-2 px-4 py-2 rounded-lg">
              {loading ? (
                <>
      ...
      <div className="space-y-6 pt-12 border-t border-slate-100">
        <div className="flex items-center justify-between text-slate-600">
          ...
          <span className="text-sm text-slate-500 bg-slate-50 px-2">{generations.length}</span>
        </div>
              ...
              <div className="space-y-6">
                    ...
                    {PLATFORMS.map((p) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-2.5 rounded-md border text-xs ${active ? "bg-red-500/80 text-white border-red-500" : "bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`}
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="relative">
                    ...
                    <input type="date" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 sm focus:outline-none transition-all" value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)} />
                  </div>
                  <div className="relative">
                    <ClockIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input type="time" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 sm focus:outline-none transition-all" value={scheduledTime} onChange={(e) => setScheduledTime(e.target.value)} />
                  </div>
                </div>
